...
*/
void NDTMap::loadPointCloud(const pcl::PointCloud<pcl::PointXYZI> &pc2, double range_limit)
{
  pcl::PointCloud<pcl::PointXYZ> pc;
  pc.points.reserve(pc2.points.size());
  for (int i = 0; i < pc2.points.size(); ++i)
    pc.points.push_back(pcl::PointXYZ(pc2.points[i].x,
				      pc2.points[i].y,
				      pc2.points[i].z));
    if(index_ != NULL)
    {
	...
	//    std::cerr<<"adding to update set\n";
	    ...
	    ptCell->intensities_.push_back(pc2.points[idx].intensity);
	}
#endif
        it++; idx++;
    }
...
void  NDTMap::addPointCloudMeanUpdate(const Eigen::Vector3d &origin,
    ...
    ndlocal.loadPointCloudCentroid(pc,origin, old_centroid, localmapsize, -1.); // No range limit used here, assume that the point cloud is already filtered.
...
std::vector< std::shared_ptr<lslgeneric::NDTCell> > NDTMap::getAllCellsShared() const
{
    ...
    while (it != index_->end())
    {
		...
		if(cell->hasGaussian_)
		{
			...
			NDTCell* nd = cell->copy();
			std::shared_ptr< NDTCell > smart_pointer(nd);
// 			NDTCell** ndd = &nd;
...
std::vector< std::shared_ptr<lslgeneric::NDTCell> > NDTMap::getAllInitializedCellsShared() const
{
    std::vector<std::shared_ptr<NDTCell> > ret;
    typename SpatialIndex::CellVectorItr it = index_->begin();
    while (it != index_->end())
...
void NDTMap::insertCell(NDTCell cell){
    LazyGrid* gr = dynamic_cast<LazyGrid*>(index_);
