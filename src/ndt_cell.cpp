...
{
  float intensity=0;
  for(unsigned int i=0; i< intensities_.size(); i++)
  {
    intensity+= ((float)intensities_[i]);
  }
  this->setIntensity(intensity/(float)intensities_.size());
}
...
void NDTCell::classify()
{
    ...
    else
    {
        ...
        double ac = d/l;
        if(std::abs(ac) < EVEC_INCLINED_THR)
        {
            //angle is nearly perpendicular => vertical surface
            cl_ = VERTICAL;
        }

        if(std::abs(ac) > 1-EVEC_INCLINED_THR)
        {
